  ...
  editCity = (e) => {
	...
	render() {
		const userCities = this.state.userCities.map((city, i) => {
			return <div key={i}>
					<li>{city.cityname}, {city.countrycode} <button value={city.id} onClick={this.editCity}>Edit</button><button value={city.id} onClick={this.deleteCity}>Delete</button></li>
					</div>
		})
		return(
			<div className="editStuff">
				...
				<div>
				<h3>Edit Current Cities</h3><button onClick={this.currentCities}>Open</button>
					<ul>
					{userCities}
